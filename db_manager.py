...
© 2025 闲鱼自动回复系统"""
    ...
    def update_item_detail(self, cookie_id: str, item_id: str, item_detail: str) -> bool:
        """更新商品详情（不覆盖商品标题等基本信息，保留 JSON 格式并更新描述列）

        Args:
            cookie_id: Cookie ID
            item_id: 商品ID
            item_detail: 商品详情JSON字符串或纯文本详情
        ...
        try:
            with self.lock:
                cursor = self.conn.cursor()
                # 检查当前库里的 item_detail 和 item_description
                cursor.execute('''
                SELECT item_detail, item_description FROM item_info
                WHERE cookie_id = ? AND item_id = ?
                ''', (cookie_id, item_id))
                row = cursor.fetchone()
                ...
                if not is_new_json:
                    ...
                    if old_is_json:
                        ...
                        cursor.execute('''
                        UPDATE item_info SET
                            item_detail = ?, item_description = ?, updated_at = CURRENT_TIMESTAMP
                        WHERE cookie_id = ? AND item_id = ?
