...
# 我们已经在宿主机编译好前端，并挂载到 /app/static，所以不需要在 Docker 内构建前端，直接用 runtime 阶段即可
# 以下不需要在 runtime 重复构建，直接复制过去
# COPY --from=frontend-builder /frontend/dist ./static
# 注意，Vite 打包直接输出到了 ../static (即 ./static)
COPY ./static ./static
...
COPY --from=builder /opt/venv /opt/venv
    # 我们已经在宿主机打包好前端，不需要在容器内再进行 frontend-builder
    # COPY --from=frontend-builder /frontend/dist ./static
    # Vite 编译产物已输出到 ./static，这里直接 COPY 整个 /app/static 即可
